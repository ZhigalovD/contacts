    ...
    <div class="create__wrapper">
      ...
      <div class="create__required-fields">
        ...
        <div class="create__required-fields--inp">
          ...
          <input type="text" name="name" placeholder="Введите значение" v-model="name" @change="dataChange">
        </div>
        <div class="create__required-fields--inp">
          <input type="text" placeholder="Номер телефона" disabled>
          <input type="text" name="phone" placeholder="Введите значение" v-model="phone" @change="dataChange">
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['ADD_INFORMATION', 'GET_REQUIRE_FIELDS', 'GET_CONTACTS', 'GET_ID']),
    Contacts() {
      return {...this.GET_ID, ...this.GET_REQUIRE_FIELDS, ...[this.ADD_INFORMATION]}
    },
  ...
  methods: {
    ...mapMutations(['SET_INFORMATION', 'SET_REQUIRE_FIELDS', 'SET_ALL_CONTACTS', 'DEFAULT_INFO', 'DEFAULT_REQUIRE_FIELDS', 'SET_INFORMATION_FROM_CONTACTS']),
    submit() {
    ...
    fetchData() {
      this.GET_CONTACTS.forEach(el => {
        if(el.id_user === this.$route.query.contact_id) {
          ...
          this.name = el.name
          this.phone = el.phone
          this.SET_REQUIRE_FIELDS({id_user: el.id_user, name: el.name, phone: el.phone})
        }
      })
    }
  }
